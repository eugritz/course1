    ...
    S: BuildHasher,
{
    ...
    pub fn get_next<Q: ?Sized>(&self, value: &Q) -> Option<&T>
    where
        T: Borrow<Q>,
        Q: Hash + Eq,
    {
        self.map.get_next_key(value)
    }

    pub fn get_prev<Q: ?Sized>(&self, value: &Q) -> Option<&T>
    where
        T: Borrow<Q>,
        Q: Hash + Eq,
    {
        self.map.get_prev_key(value)
    }

    pub fn take_next<Q: ?Sized>(&mut self, value: &Q) -> Option<T>
    where
        T: Borrow<Q>,
        Q: Hash + Eq,
    {
        self.map.take_next(value).map(|e| e.0)
    }

    pub fn take_prev<Q: ?Sized>(&mut self, value: &Q) -> Option<T>
    where
        T: Borrow<Q>,
        Q: Hash + Eq,
    {
        self.map.take_prev(value).map(|e| e.0)
    }
...
impl<T: Hash + Eq + Clone, S: BuildHasher + Clone> Clone
    for LinkedHashSet<T, S>
{
    fn clone(&self) -> Self {
